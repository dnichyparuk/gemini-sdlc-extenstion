...
try {
  ...
  if (transcriptBuffer.includes('Plan mode is active')) {
    process.stdout.write(JSON.stringify({
      systemMessage: `[plan-integrity] WARNING: Plan presented but plan-sdlc was not invoked (no plan integrity state for branch ${branch}). Quality gates may have been bypassed.`
    }) + '\n');
  }
