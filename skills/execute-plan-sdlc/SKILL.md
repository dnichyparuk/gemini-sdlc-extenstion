...
description: "Executes implementation plans with adaptive intelligence. Classifies tasks, builds waves, verifies results, and recovers from failures autonomously. Handles parallel task execution and spec compliance."
user-invocable: true
...
- **Model**: pass `model: "<assigned-model>"` to the `invoke_agent` tool (haiku, sonnet, or opus per the selected quality tier)
  **This is REQUIRED on every dispatch — no exceptions.** Omitting `model:` causes the agent to inherit the parent model (opus), defeating the quality-tier system. Before sending any Agent dispatch message, verify: does every `invoke_agent` call in this message include `model:`? If not, add it.
- **Mode**: pass `mode: "bypassPermissions"` to the `invoke_agent` tool on every dispatch.
...
**State persistence:** After each wave completes, update the execution state via `scripts/state/execute.js`. Locate the script:
```bash
STATE_SCRIPT="scripts/state/execute.js"
[ ! -f "$STATE_SCRIPT" ] && { echo "ERROR: Could not locate $STATE_SCRIPT."; exit 2; }
```
