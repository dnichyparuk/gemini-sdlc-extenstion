...
tools: [read_file, write_file, glob, grep_search, run_shell_command, invoke_agent]
...
Resolve REFERENCE.md: `glob` with `dir_path: ~/.claude` and pattern `**/review-sdlc/REFERENCE.md`.
If not found, retry `glob` with `dir_path: PROJECT_ROOT`. Store the resolved absolute path as
...
1. Read the pre-computed diff: `read_file(dimension.diff_file)`
...
3. Dispatch via `invoke_agent` tool (agent_name: codebase_investigator or similar, but instructions say general-purpose. Mapping to `generalist`).
...
**Dispatch ALL active dimensions in a SINGLE message** (multiple `invoke_agent` tool calls in
...
**Persist the comment body to disk** using the `write_file` tool:
